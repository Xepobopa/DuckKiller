...
Game::Game(AssertManager& manager, Save& save)
{
	// set background
	...
	// set amount of coins from save
	//std::cout << ;
	if (save.readData().is_null()) { this->coins = 0; }
	else { this->coins = save.readData()["coins"]; }

	// set coins' text
	this->coinText = new sf::Text("Coins: ", *manager.getTextFont(), 23);
	setCoin(coins); // and set all coins that player have 
...
void Game::setCoin(int coin)
{
	this->coins = coin;
	this->coinText->setString("Coins " + std::to_string(coin));
}

void Game::addCoins(int coins)
{
	this->coins += coins;
	this->coinText->setString("Coins " + std::to_string(this->coins));
}

sf::Text* Game::getCoinText()
{
	return coinText;
}
